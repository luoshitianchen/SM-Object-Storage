# 对象存储服务 上线部署检查清单（DEPLOYMENT_CHECKLIST）
...
- [ ] ConfigMap 环境变量正确（SM_ENV / SM_DATABASE_NAME=sm_object_storage）
...
1. `helm lint helm/sm-object-storage`
2. `helm upgrade --install sm-object-storage helm/sm-object-storage -n sm-prod -f helm/sm-object-storage/values-prod.yaml`
3. 等待 rollout：`kubectl -n sm-prod rollout status deploy/sm-object-storage`
